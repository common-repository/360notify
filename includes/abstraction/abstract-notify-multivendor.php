	...
	protected function perform_grouping( $vendor_data_list ) {
		...
		foreach ( $group_vendor_datas as $phone_number => $vendor_datas ) {
			...
			foreach ( $vendor_datas as $vendor_data ) {
				...
				$this->log->add( '360MessengerWhatsApp', 'item data for vendor id (' . $vendor_data['vendor_user_id'] . ') : ' . json_encode( $vendor_data['item']->get_data() ) );
			}
		...
		$this->log->add( '360MessengerWhatsApp', 'processed data: ' . json_encode( $new_group_vendor_datas ) );
