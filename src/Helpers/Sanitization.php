...
class Sanitization {
     ...
     */
    static function notifyapi_sanitize_array( $arr ) {
        global $wp_version;
        ...
        foreach ( $arr as $key => $val ) {
            $result[ $key ] = is_array( $val ) ? Sanitization::notifyapi_sanitize_array( $val ) : ( ( $older_version ) ? stripcslashes( sanitize_text_field( $val ) ) : stripcslashes( sanitize_textarea_field( $val ) ) );
        }
