...
let caretPosition = 0;
jQuery(function ($) {
    $("#notifysms_setting_form").on("submit", function (event) {
        var sender_id = $("#notifysms_setting\\[notifysms_woocommerce_sms_from\\]").val().trim();
        if ($.isNumeric(sender_id) && sender_id.length > 15) {
            alert('Message From is too long, max 15 digits for numeric WhatsApp sender.');
            event.preventDefault();
            return;
        } else if (!$.isNumeric(sender_id) && sender_id.length > 11) {
            alert('Message From is too long, max 11 characters for alphanumeric WhatsApp sender.');
            event.preventDefault();
            return;
        }
        $("#notifysms_setting\\[notifysms_woocommerce_sms_from\\]").val(sender_id);
